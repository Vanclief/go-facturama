...
func (request *CreateCfdiV4Request) Validate() error {
	...
	if request.ExpeditionPlace == "" {
		return ez.New(ez.EINVALID, "ExpeditionPlace is required", nil)
	}
	if request.Folio == "" {
		return ez.New(ez.EINVALID, "Folio is required", nil)
	}
	if request.CfdiType == "" {
		return ez.New(ez.EINVALID, "CfdiType is required", nil)
	}

	// Validate CfdiType (I|E|T|N|P)
	if !regexp.MustCompile(`^[IETNP]$`).MatchString(request.CfdiType) {
		return ez.New(ez.EINVALID, "CfdiType must be one of: I, E, T, N, P", nil)
	}

	// Validate ExpeditionPlace (5 digit zip code)
	if !regexp.MustCompile(`^[0-9]{5}$`).MatchString(request.ExpeditionPlace) {
		return ez.New(ez.EINVALID, "ExpeditionPlace must be a 5-digit zip code", nil)
	}

	// Validate Folio (1-40 chars)
	if len(request.Folio) < 1 || len(request.Folio) > 40 {
		return ez.New(ez.EINVALID, "Folio must be between 1 and 40 characters", nil)
	}
	...
	if request.PaymentForm != "" {
		...
		if !isValid {
			return ez.New(ez.EINVALID, "Invalid PaymentForm value", nil)
		}
	}

	if request.PaymentMethod != "" {
		if request.PaymentMethod != "PUE" && request.PaymentMethod != "PPD" {
			return ez.New(ez.EINVALID, "PaymentMethod must be either PUE or PPD", nil)
		}
	}

	// Validate Issuer
	if request.Issuer.Rfc == "" {
		return ez.New(ez.EINVALID, "Issuer.Rfc is required", nil)
	}
	if request.Issuer.FiscalRegime == "" {
		return ez.New(ez.EINVALID, "Issuer.FiscalRegime is required", nil)
	}

	// Validate Receiver
	if request.Receiver.Rfc == "" {
		return ez.New(ez.EINVALID, "Receiver.Rfc is required", nil)
	}
	if request.Receiver.Name == "" {
		return ez.New(ez.EINVALID, "Receiver.Name is required", nil)
	}
	if request.Receiver.CfdiUse == "" {
		return ez.New(ez.EINVALID, "Receiver.CfdiUse is required", nil)
	}
	if request.Receiver.FiscalRegime == "" {
		return ez.New(ez.EINVALID, "Receiver.FiscalRegime is required", nil)
	}
	if request.Receiver.TaxZipCode == "" {
		return ez.New(ez.EINVALID, "Receiver.TaxZipCode is required", nil)
	}

	// Validate Items
	if len(request.Items) == 0 {
		return ez.New(ez.EINVALID, "At least one item is required", nil)
	}

	for i, item := range request.Items {
		if item.ProductCode == "" {
			return ez.New(ez.EINVALID, fmt.Sprintf("Items[%d].ProductCode is required", i), nil)
		}
		if item.Description == "" {
			return ez.New(ez.EINVALID, fmt.Sprintf("Items[%d].Description is required", i), nil)
		}
		if item.Unit == "" {
			return ez.New(ez.EINVALID, fmt.Sprintf("Items[%d].Unit is required", i), nil)
		}
		if item.UnitCode == "" {
			return ez.New(ez.EINVALID, fmt.Sprintf("Items[%d].UnitCode is required", i), nil)
		}
		if item.TaxObject == "" {
			return ez.New(ez.EINVALID, fmt.Sprintf("Items[%d].TaxObject is required", i), nil)
		}
	}
